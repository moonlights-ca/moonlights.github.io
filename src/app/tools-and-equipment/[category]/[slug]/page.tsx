...
export function generateStaticParams() {
  return toolsSystem.categories.flatMap((cat) =>
    cat.products.map((p) => ({
      category: cat.slug,
      slug: p.slug,
    }))
...
export async function generateMetadata({
  ...
  const url = canonicalUrl(`${toolsSystem.basePath}/${category}/${slug}`);
  return {
...
export default async function ProductDetailPage({
  ...
  return <CategoryProductDetail system={toolsSystem} category={cat} product={product} />;
}
