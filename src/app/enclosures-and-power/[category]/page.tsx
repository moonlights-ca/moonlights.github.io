...
export function generateStaticParams() {
  return enclosuresCategories.map((cat) => ({
    category: cat.slug,
  }));
}
...
export async function generateMetadata({
  ...
  return {
    title: `${cat.name} | Distribution & Protection | Moonlights Electrical`,
      ...
      <section className="pb-stack-xl">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-gutter">
          {cat.products.map((product) => (
            <Link
              key={product.slug}
              href={`/enclosures-and-power/${category}/${product.slug}`}
      ...
      <section className="pb-stack-xl">
        <div className="border-t border-outline-variant/20 pt-8 flex items-center">
          ...
          >
            ...
            Back to All Distribution & Protection
          </Link>
        </div>
      </section>
    </main>
