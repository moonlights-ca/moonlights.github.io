...
export default function ConduitFittings() {
 ...
 <span className="text-primary">CONDUIT & FITTINGS</span>
 </nav>
 </div>
 ...
 EMT, rigid, PVC, and liquidtight conduit systems — complete with fittings, boxes, enclosures, cable tray, and motor control components for any installation.
 </p>
 </div>
 </section>
 ...
 {datacomCategories.length > 0 && (
 <Link
 href={`/datacom-and-security/${datacomCategories[0].slug}`}
 ...
 src={datacomCategories[0].img}
 fill
 sizes="(max-width: 768px) 100vw, 33vw"
 loading="lazy"
 />
 <div className="absolute top-4 left-4">
 <span className="chip backdrop-blur-md px-3 py-1 rounded text-on-surface font-label-caps text-label-caps lm-bg-black lm-text-white">Raceway Systems</span>
 </div>
 </div>
 <div className="p-8 md:w-1/2 flex flex-col justify-center gap-4">
 <h2 className="font-headline-md text-headline-md bg-gradient-to-r from-primary to-secondary-fixed bg-clip-text text-transparent lm-text-black">{datacomCategories[0].name}</h2>
 <p className="font-body-md text-body-md text-on-surface-variant lm-text-black">
 {datacomCategories[0].desc}
 </p>
 <div className="mt-4">
 ...
 {datacomCategories.slice(1).map((cat) => (
 <Link
 key={cat.slug}
 href={`/datacom-and-security/${cat.slug}`}
