...
export default function TransformerCatalogGrid() {
  ...
  const filtered = useMemo(() => {
    return transformerProducts.filter((p) => {
      ...
      <section className="mb-8">
        ...
        <div className="flex items-center gap-2 pt-3">
          ...
          <span className="font-body-md text-body-md text-on-surface-variant">
            Showing <span className="text-on-surface font-semibold">{filtered.length}</span> of {transformerProducts.length} products
          </span>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-gutter">
            {filtered.map((product) => (
              <Link
                key={product.slug}
                href={`/transformers/${product.slug}`}
