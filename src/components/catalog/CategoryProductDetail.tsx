...
import type { CategoryProduct, CategorySystem, ProductCategory } from "@/lib/products";

export interface CategoryProductDetailProps {
...
/**
 * Detail layout for a single category product, including previous/next
 * navigation within the category. Shared by all category-based sections.
 */
export default function CategoryProductDetail({ system, category, product }: CategoryProductDetailProps) {
  const categoryPath = `${system.basePath}/${category.slug}`;

  // Find adjacent products for navigation
  const idx = category.products.findIndex((p) => p.slug === product.slug);
  const prev = idx > 0 ? category.products[idx - 1] : null;
  const next = idx < category.products.length - 1 ? category.products[idx + 1] : null;
    ...
    <main className="flex-grow w-full max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop relative z-10">
      <JsonLd
        data={[
          productSchema({
            ...
            image: product.img,
            path: `${categoryPath}/${product.slug}`,
          ...
          breadcrumbSchema([
            ...
            { name: product.name, path: `${categoryPath}/${product.slug}` },
          ]),
        ]}
      />
      {/* Breadcrumb */}
      <Breadcrumb
        items={[
          ...
          { label: category.name.toUpperCase(), href: categoryPath },
