...
export interface CatalogProductDetailProps {
  ...
  /** Route prefix for the parent catalog, e.g. `/wires-and-cables`. */
  basePath: string;
  /** Uppercase section label shown in the breadcrumb, e.g. `WIRES & CABLES`. */
  sectionLabel: string;
  /** Label for the "back to catalog" link, e.g. `Back to All Wires & Cables`. */
  backLabel: string;
  /** Labels for the two quick-spec fields rendered from `tags[0]` and `tags[1]`. */
  quickSpecLabels: [string, string];
}

/**
 * Full product detail layout shared by the wires & cables and transformers
 * catalogs: hero with quick specs, description, feature/application cards, and a
 * back link.
 */
      ...
      <JsonLd
        data={[
          ...
          breadcrumbSchema([
            ...
            { name: product.name, path: `${basePath}/${product.slug}` },
          ]),
        ]}
      />
      {/* Breadcrumb */}
            ...
            <div className="glass-card rounded-xl p-5">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div className="flex flex-col gap-0.5">
                  <span className="font-mono text-[10px] text-outline uppercase tracking-wider">{quickSpecLabels[0]}</span>
                  <span className="text-on-surface font-medium">{product.tags[0]}</span>
                </div>
                <div className="flex flex-col gap-0.5">
                  <span className="font-mono text-[10px] text-outline uppercase tracking-wider">{quickSpecLabels[1]}</span>
