...
export function generateStaticParams() {
  return datacomSystem.categories.flatMap((cat) =>
    cat.products.map((p) => ({
      category: cat.slug,
      slug: p.slug,
    }))
...
export async function generateMetadata({
  ...
  const url = canonicalUrl(`${datacomSystem.basePath}/${category}/${slug}`);
  return {
...
export default async function ProductDetailPage({
  ...
  return <CategoryProductDetail system={datacomSystem} category={cat} product={product} />;
}
